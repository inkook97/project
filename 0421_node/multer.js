...
// 특정 폴더를 요청에 의해 직접 파일에 접근할 수 있도록 제공하는 익스프레스 미들웨어
// npm i serve-static
...
// 특정사이트에 접속했을때 마다 정보(접속시간등등) 확인하는 모듈

// npm i multer
const multer = require('multer');
// 파일을 어로드 하기 위한 익스프레스 미들웨어
...
// multer,, 업로드 하는 파일에 대한 설정
const storage = multer.diskStorage({
    destination: (req, file, callback) => { // 파일을 저장할 디렉토리 설정
        callback(null, 'uploads');
    },
    filename: (req, file, callback) => { // 업로드 되는 파일명 설정
        ...
        callback(null, basename + "_" + Date.now() + extension); // 파일명_34588348.png
    }
});


const upload = multer({
    storage: storage, // 업로드 되는 폴더와 파일명 설정
    limit: {
        files: 5, // 파일전송개수
